...
export default async function BookPage({ params }: { params: Promise<{ bookId: string }> }) {
  ...
  return (
    <main className="container mx-auto">
      <section className="relative mt-[10vh] flex h-[70vh] rounded-2xl bg-white">
        ...
        <section className="flex-1">
          <h1 className="mt-15 mb-8 flex items-center justify-center gap-x-4 text-center text-4xl">
          ...
          <ScrollArea className="h-[calc(100%-175px)]">
            <section className="mx-10 grid h-[calc(100%-150px)] grid-cols-3 gap-5 pt-2 pb-1">
              {bookData.map((book) => {
                const [lesson] = book.filename.split('－');
                return (
                  <Link key={book.title} href={`/${bookId}/${book.filename}`}>
                    <Card className="gap-y-1 border border-white bg-linear-0 from-amber-100 to-amber-400 p-0.5 shadow-none ring-0 outline-0 transition-all duration-300 ease-in-out hover:-translate-y-1 hover:cursor-pointer hover:border-amber-400 hover:shadow-md">
                      <CardContent className="flex h-14 items-center justify-center gap-x-4 rounded-lg border border-amber-400/50 bg-white/95 px-2 text-center text-sm font-semibold wrap-anywhere break-keep text-gray-700">
                        {book.title}
                      </CardContent>
